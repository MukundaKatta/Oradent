...
export function InsuranceClaimForm({ open, onClose, onSave }: InsuranceClaimFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      diagnosisCodes: '',
      procedures: [
        { cdtCode: '', toothNumber: '', surfaces: '', fee: 0, date: new Date().toISOString().split('T')[0] },
      ],
      notes: '',
    },
  });
  ...
  const onSubmit = async (data: ClaimFormData) => {
    setSaving(true);
    try {
      await apiPost('/api/billing/claims', data);
      onSave();
    } catch (error) {
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-stone-200 px-3 py-2 text-sm focus:border-teal-500 focus:outline-none focus:ring-1 focus:ring-teal-500"
                />
                {errors.providerNpi && <p className="mt-1 text-xs text-red-500">{errors.providerNpi.message}</p>}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {fields.map((field, index) => (
                  <div key={field.id} className="grid grid-cols-[90px_70px_70px_90px_100px_32px] gap-2 items-end">
                    <div>
                      {index === 0 && <span className="mb-1 block text-xs text-stone-400">CDT</span>}
                    ...
                    <div>
                      ...
                      <button type="button" onClick={() => fields.length > 1 && remove(index)} disabled={fields.length <= 1} className="rounded p-1.5 text-stone-400 hover:text-red-500 disabled:opacity-30">
